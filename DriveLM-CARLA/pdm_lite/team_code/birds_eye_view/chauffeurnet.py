...
class ObsManager(ObsManagerBase):
    ...
    def _warp_static_map(self, source, m_warp):
        """Warp only the source region needed by the local BEV output."""
        width = self._width
        inverse = cv.invertAffineTransform(m_warp)
        ...
        x0 = max(
            0,
            int(np.floor(source_corners[:, 0].min())) - padding,
        )
        y0 = max(
            0,
            int(np.floor(source_corners[:, 1].min())) - padding,
        )
        x1 = min(
            source.shape[1],
            int(np.ceil(source_corners[:, 0].max())) + padding + 1,
        )
        y1 = min(
            source.shape[0],
            int(np.ceil(source_corners[:, 1].max())) + padding + 1,
        )
        ...
        local_warp = m_warp.copy()

        local_warp[:, 2] += (
            local_warp[:, :2]
            @ np.array([x0, y0], dtype=local_warp.dtype)
        )
    ...
    def get_road(self):
        ...
        # road_mask, lane_mask
        road_mask = self._warp_static_map(self._road, m_warp).astype(bool)
        lane_mask_all = self._warp_static_map(
            self._lane_marking_all, m_warp
        ).astype(bool)
        lane_mask_broken = self._warp_static_map(
            self._lane_marking_white_broken, m_warp
        ).astype(bool)
    ...
    def get_observation(self, close_traffic_lights=None):
        ...
        # road_mask, lane_mask
        road_mask = self._warp_static_map(self._road, m_warp).astype(bool)
        sidewalk_mask = self._warp_static_map(self._sidewalk, m_warp).astype(bool)
        lane_mask_all = self._warp_static_map(
            self._lane_marking_all, m_warp
        ).astype(bool)
        lane_mask_broken = self._warp_static_map(

...
from mesh_tracking_collector_v2_candidate import MeshTrackingCollector
...
class DataAgent(AutoPilot):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        self.mesh_tracking_collector = None
        self.mesh_tracking_enabled = (
            int(os.environ.get("MESH_TRACKING", 0)) == 1
        )
    ...
    def tick(self, input_data):
        ...
        if self.save_path is not None and (self.datagen or self.tmp_visu):
            ...
            depth_raw = input_data["depth"][1][:, :, :3].copy()
            depth = (
                t_u.convert_depth(depth_raw) * 255.0 + 0.5
            ).astype(np.uint8)

            instance_raw = input_data["instance"][1][:, :, :3].copy()
            optical_flow = input_data.get(
                "optical_flow", (None, None)
            )[1]
            if optical_flow is not None:
                optical_flow = optical_flow.copy()
    ...
    def run_step(self, input_data, timestamp, sensors=None, plant=False):
        ...
        if self.mesh_tracking_enabled and self.datagen:
            self._update_mesh_tracking(
                input_data, tick_data, timestamp, sensors
            )
    ...
    def _update_mesh_tracking(
        self, input_data, tick_data, timestamp, sensors
    ):
        if self.save_path is None:
            return

        required = ["rgb", "depth", "instance", "semantics"]
        if tick_data.get("optical_flow") is not None:
            required.append("optical_flow")
        frame_ids = [int(input_data[name][0]) for name in required]

        if len(set(frame_ids)) != 1:
            raise RuntimeError(
                "mesh tracking sensor frame mismatch: {}".format(
                    dict(zip(required, frame_ids))
                )
            )

        if self.mesh_tracking_collector is None:
            sensor_map = dict(sensors or [])
            camera_sensor = sensor_map.get("depth")

            if camera_sensor is None:
                raise RuntimeError(
                    "mesh tracking cannot find the depth sensor actor"
                )

            output_root = (
                self.save_path / "mesh_tracking_clips"
            )

            self.mesh_tracking_collector = MeshTrackingCollector(
                root=output_root,
                world=self._world,
                ego=self._vehicle,
                camera_sensor=camera_sensor,
                width=tick_data["depth_raw"].shape[1],
                height=tick_data["depth_raw"].shape[0],
                fov=self.config.camera_fov,
            )

            print(
                "mesh tracking: route collector initialized at {}".format(
                    output_root
                )
            )

        self.mesh_tracking_collector.process(
    ...
    def destroy(self, results=None):
        if getattr(self, "mesh_tracking_collector", None) is not None:
            self.mesh_tracking_collector.close(
